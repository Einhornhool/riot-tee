...
CYS_error_t cc310_aes_128_encrypt_decrypt(uint32_t *key, uint32_t *iv, uint8_t *input, size_t input_len, uint8_t *output, size_t output_len, int mode)
{
    ...
    NRF_CC_AES->AES_CONTROL = mode;

    /* Load the AES key value into the engine */
    for (int i = 0; i < 4; i++) {
        NRF_CC_AES->AES_KEY_0[i] = key[i];
    }

    if (iv) {
        for (int i = 0; i < 4; i++) {
            NRF_CC_AES->AES_IV_0[i] = iv[i];
        }
    }
    else {
        /* If IV == NULL, set IV to 0 */
        for (int i = 0; i < 4; i++) {
            NRF_CC_AES->AES_IV_0[i] = 0x00;
        }
    }

    /* Configure DMA output destination address */
    NRF_CC_DOUT->DST_MEM_ADDR = (uint32_t) output;
    NRF_CC_DOUT->DST_MEM_SIZE = (uint32_t) output_len;
